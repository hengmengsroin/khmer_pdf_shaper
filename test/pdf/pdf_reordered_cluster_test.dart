...
void main() {
  group('Reordered-Cluster ToUnicode Ownership Regression Tests', () {
    ...
    test('Cluster ownership for ក្រ (reordered pre-base subscript Ro)', () {
      ...
      final semanticText =
          text.substring(cluster.sourceStart, cluster.sourceEnd);
      ...
      final c0 = registry.allocate(
        ...
        subsetGlyphId: 1,
        unicodeText:
            semanticText, // Primary visual glyph receives full cluster Unicode
      );
    ...
    test('Cluster ownership for គ្រែ (pre-base vowel + pre-base subscript Ro)',
      ...
      expect(
          cluster.glyphs[1].glyphId, equals(205)); // Subscript Ro (source 1..3)
    ...
    test(
        'Cluster ownership for ខ្ញុំ (complex cluster: base + coeng Nyo + vowel U + Anusvara)',
        () {
    ...
    test(
        'Cluster ownership for multi-syllable word សួស្តី (2 syllables: សួ + ស្តី)',
        () {
      ...
      expect(c2.glyphs.length,
          equals(3)); // Sa (84), Coeng Ta (307), Vowel Ii (277)
      ...
      final entrySa1 =
          font.registry.entries.values.firstWhere((e) => e.unicodeText == 'សួ');
      final entrySa2 = font.registry.entries.values
          .firstWhere((e) => e.unicodeText == 'ស្តី');
