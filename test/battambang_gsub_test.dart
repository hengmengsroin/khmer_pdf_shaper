...
void main() {
  group('Part 21: Battambang GSUB Targeted Cases Tests', () {
    ...
    test('Case 3: គ្រែ (COENG RO + Pre-base vowel)', () {
      ...
      expect(
          run.glyphs.map((g) => g.xAdvance).toList(), [610.0, 610.0, 1221.0]);
    });
    ...
    test('Case 4: កោ (Split vowel OA)', () {
      ...
      expect(
          run.glyphs.map((g) => g.xAdvance).toList(), [610.0, 1221.0, 610.0]);
    });
    ...
    test('Case 5: ខ្ញុំ (Subscript + Above mark + Below mark)', () {
      ...
      expect(
          run.glyphs.map((g) => g.xAdvance).toList(), [1221.0, 0.0, 0.0, 0.0]);
    });

    test('Case 6: សួស្តី (Multi-syllable word)', () {
      final run = shaper.shapeText('សួស្តី');
      expect(
          run.glyphs.map((g) => g.glyphId).toList(), [84, 282, 84, 307, 277]);
      expect(run.glyphs.map((g) => g.xAdvance).toList(),
          [1831.0, 0.0, 1831.0, 0.0, 0.0]);
    });

    test('Case 7: កម្ពុជា (Country name)', () {
      final run = shaper.shapeText('កម្ពុជា');
      expect(run.glyphs.map((g) => g.glyphId).toList(),
          [53, 77, 313, 329, 136, 121]);
      expect(run.glyphs.map((g) => g.xAdvance).toList(),
          [1221.0, 1221.0, 0.0, 0.0, 1221.0, 610.0]);
    });

    test('Case 8: សង្គ្រាម (Multiple subscripts + COENG RO + post vowel)', () {
      final run = shaper.shapeText('សង្គ្រាម');
      expect(run.glyphs.map((g) => g.glyphId).toList(),
          [84, 207, 130, 297, 121, 77]);
      expect(run.glyphs.map((g) => g.xAdvance).toList(),
          [1831.0, 610.0, 1221.0, 0.0, 610.0, 1221.0]);
    });
