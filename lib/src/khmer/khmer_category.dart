...
enum KhmerCategory {
  ...
  placeholder(10, 'PLACEHOLDER', 'Placeholder / Dotted Circle substitute / Digits'),
...
KhmerCategory getKhmerCategory(int codePoint) {
  ...
  if (codePoint == 0x2015 || codePoint == 0x2022) return KhmerCategory.placeholder;
  if (codePoint == 0x25CC) return KhmerCategory.dottedCircle;
  if (codePoint >= 0x25FB && codePoint <= 0x25FE) return KhmerCategory.placeholder;
  ...
  if (codePoint >= 0x1780 && codePoint <= 0x17FF) {
    ...
    if (codePoint >= 0x17B7 && codePoint <= 0x17BA) return KhmerCategory.vowelAbove;
    if (codePoint >= 0x17BB && codePoint <= 0x17BD) return KhmerCategory.vowelBelow;
    ...
    if (codePoint >= 0x17BF && codePoint <= 0x17C0) return KhmerCategory.vowelPost;
    if (codePoint >= 0x17C1 && codePoint <= 0x17C3) return KhmerCategory.vowelPre;
    if (codePoint >= 0x17C4 && codePoint <= 0x17C5) return KhmerCategory.vowelPost;

    // Marks & Modifiers
    if (codePoint == 0x17C6) return KhmerCategory.xGroup; // Nikahit (Bindu)
    if (codePoint >= 0x17C7 && codePoint <= 0x17C8) return KhmerCategory.yGroup; // Reahmuk, Yuukaleapintu
    if (codePoint >= 0x17C9 && codePoint <= 0x17CA) return KhmerCategory.robatic; // Muusikatoan, Triisap
    if (codePoint == 0x17CB) return KhmerCategory.xGroup; // Bantoc
    if (codePoint == 0x17CC) return KhmerCategory.robatic; // Robat
    if (codePoint >= 0x17CD && codePoint <= 0x17D1) return KhmerCategory.xGroup; // Toandakhiat, Kakabat, Ahsda, Samyok Sannya, Viriam
    if (codePoint == 0x17D2) return KhmerCategory.coeng; // Coeng (Stacker/Virama)
    ...
    if (codePoint >= 0x17E0 && codePoint <= 0x17E9) return KhmerCategory.placeholder;
