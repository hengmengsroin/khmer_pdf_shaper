...
void main() {
  ...
  for (final f in fixtures) {
    ...
    if (pureRun.glyphs.length != expectedGlyphs.length) {
      pureMatches = false;
      pureDartMismatchDetails.add(
          '$id: count mismatch pure=${pureRun.glyphs.length} exp=${expectedGlyphs.length}');
    } else {
      for (int i = 0; i < pureRun.glyphs.length; i++) {
        ...
        if (actual.glyphId != exp['glyph_id'] ||
          ...
          pureMatches = false;
          pureDartMismatchDetails
              .add('$id glyph $i mismatch: pure=$actual, exp=$exp');
          break;
        }
      }
    }
    ...
    bufferSetClusterLevel(
        buffer, 0); // HB_BUFFER_CLUSTER_LEVEL_MONOTONE_GRAPHEMES = 0
    ...
    if (count != expectedGlyphs.length) {
      ffiMatch = false;
      ffiMismatchDetails
          .add('$id: count mismatch ffi=$count exp=${expectedGlyphs.length}');
    } else {
      for (int i = 0; i < count; i++) {
        ...
        if (info.codepoint != exp['glyph_id'] ||
          ...
          ffiMatch = false;
          ffiMismatchDetails.add(
              '$id glyph $i mismatch: ffi=(gid:${info.codepoint}, cl:${info.cluster}, adv:(${pos.x_advance},${pos.y_advance}), off:(${pos.x_offset},${pos.y_offset})) vs exp=$exp');
          break;
        }
      }
    }
  ...
  print(
      'Pure Dart vs Oracle:  $pureDartMatches exact, $pureDartMismatches failed');
