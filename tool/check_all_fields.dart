...
void main() {
  ...
  for (final f in fixtures) {
    ...
    if (run.glyphs.length != expectedGlyphs.length) {
      match = false;
      mismatches.add(
          '$id: length mismatch (${run.glyphs.length} vs ${expectedGlyphs.length})');
    } else {
      for (int i = 0; i < run.glyphs.length; i++) {
        ...
        if (actual.glyphId != expected['glyph_id'] ||
          ...
          final expectedClusters =
              expectedGlyphs.map((g) => g['cluster']).toList();
          mismatches.add(
              '$id ("$text"): actual clusters $actualClusters vs expected $expectedClusters');
          break;
        }
      }
    }
