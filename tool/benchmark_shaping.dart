...
typedef hb_buffer_reset_func = ffi.Void Function(
    ffi.Pointer<ffi.Opaque> buffer);
...
void main() {
  ...
  final bufferDestroy =
      dylib.lookupFunction<hb_buffer_destroy_func, HbBufferDestroy>(
          'hb_buffer_destroy');
  final bufferReset = dylib
      .lookupFunction<hb_buffer_reset_func, HbBufferReset>('hb_buffer_reset');
  ...
  swColdDart.stop();
  print(
      'Pure Dart Cold Initialization (Font parse + GSUB tables): ${swColdDart.elapsedMicroseconds} µs (${swColdDart.elapsedMilliseconds} ms)');
  ...
  swColdFfi.stop();
  print(
      'HarfBuzz FFI Cold Initialization (Blob + Face + Font + Tables): ${swColdFfi.elapsedMicroseconds} µs (${swColdFfi.elapsedMilliseconds} ms)\n');

  final testParagraph =
      'សួស្តីពិភពលោក ខ្ញុំស្រឡាញ់ប្រទេសកម្ពុជា។ ភាសាខ្មែរជាភាសាជាតិដ៏ចំណាស់ និងសម្បូរបែប។';
  ...
  for (final count in iterations) {
    ...
    print(
        '--- Benchmark: $count shape() calls (${testParagraph.length} UTF-16 code units) ---');
    print(
        '  Pure Dart:    total ${swDart.elapsedMicroseconds} µs (${swDart.elapsedMilliseconds} ms) | avg: ${avgDartUs.toStringAsFixed(2)} µs/shape');
    print(
        '  HarfBuzz FFI: total ${swFfi.elapsedMicroseconds} µs (${swFfi.elapsedMilliseconds} ms) | avg: ${avgFfiUs.toStringAsFixed(2)} µs/shape');
    print(
        '  Speed ratio:  HarfBuzz FFI is ${(avgDartUs / avgFfiUs).toStringAsFixed(2)}x faster than Pure Dart\n');
